...
- Agent po [`../middleware-policy/`](../../day-4/middleware-policy/lab-middleware-pipeline.md):
  grounding, akce, pipeline s pre/post. **Srovnej si startovní čáru** proti
  [`../middleware-policy/solution/agent.ts`](../../day-4/middleware-policy/solution/agent.ts) —
...
```powershell
cd <klon-repa>\day-5\evaluation-quality\solution
node --test
```
...
```powershell
node <klon-repa>/day-5/perf-cost-lifecycle/usage-report.mjs usage-log.jsonl --users 200 --dotazu 8
```
...
```powershell
node <klon-repa>/day-5/perf-cost-lifecycle/usage-report.mjs usage-log.jsonl `
