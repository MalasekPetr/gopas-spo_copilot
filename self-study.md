...
| [`opt-custom-retrieval`](opt-custom-retrieval/) | vedlejší kolej kurzu; plánované 30min demo na D5 **padlo při třetí rekalibraci** — zájem skupiny pokryl ŽIVÝ semantic index s ACL odučený v `knowledge-grounding` (D3) | chunking, embeddings, hybrid ranking, security trimming — a hlavně *kdy retrieval nedělat sám* |
| [`agent-framework/lab-multi-agent-triage.md`](agent-framework/lab-multi-agent-triage.md) | jen lab (modul se učí v kompaktu 45 min na D5) — třetí rekalibrace | ruční orchestrace triage + resolver nad Agents SDK, měření ceny multi-agent rozhodnutí |
